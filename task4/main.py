...
from task3.s3_client import ensure_bucket, s3_put_json, s3_key_for_report, cdn_url_for_key, s3_exists
...
def get_report_link(
    ...
    # если watermark нет — можно считать, что данных нет
    if wm is None:
        raise HTTPException(status_code=409, detail={"message": "Data not processed yet"})

    ensure_period_processed(wm, to_date)

    key = s3_key_for_report(user_id, from_date.isoformat(), to_date.isoformat(), wm.isoformat())
    url = cdn_url_for_key(key)

    if s3_exists(key):
        return ReportLinkResponse(url=url, cache="hit", available_to=wm)

    # MISS -> генерим отчёт из витрины (как раньше делал /reports)
    q = """
    SELECT
        day, prosthesis_id, steps_sum, active_minutes, avg_load, max_load,
        errors_count, battery_min, battery_avg
    FROM reports_user_daily_v2
    WHERE user_id = %(user_id)s
      AND day >= toDate(%(from)s)
      AND day <= toDate(%(to)s)
    ORDER BY day
    """
    res = ch.query(q, parameters={"user_id": user_id, "from": from_date.isoformat(), "to": to_date.isoformat()})
    ...
    # кладём в S3
    ensure_bucket()
    s3_put_json(key, report_payload)

    return ReportLinkResponse(url=url, cache="miss", available_to=wm)